...
- **`planos_mensalista_pausas`** (21/09/2026, `20260921100000_mensalista_pausa_schema.sql` + `..._mensalista_pausa_atomic.sql` + `..._mensalista_pausa_bookings_skip.sql`): pausa reversível de uma recorrência (viagem/afastamento) — período (`pausa_inicio`/`pausa_fim`), `cobranca_modo` (`integral`|`proporcional`|`nenhuma`), `bookings_acao` (`liberar`|`manter`), `status` (`ativa`|`cancelada`). Índice único parcial garante 1 pausa ativa por plano; a criação supera uma pausa futura existente (edição). RLS igual a `planos_mensalista_reajustes` (SELECT via `can_access_arena_backoffice`, escrita só por RPC). `planos_mensalista.status` não ganha um valor `pausado` — o plano continua `ativo`, e "pausado" é derivado de existir uma pausa `ativa`. `_insert_monthly_plan_month_bookings` consulta essa tabela para não criar reservas dentro do período pausado. Detalhe em `docs/SPEC-Web-Gestor.md` §31.
