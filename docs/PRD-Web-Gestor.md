...
#### 5.5.2 Calendário de um espaço — visão de Mês (acréscimo)
- **Status:** Implementado (15/09/2026).
- O calendário de um único espaço (`/dashboard/arenas/{id}/courts/{courtId}/calendar`) já tinha as visões **Dia** e **Semana** — grade de horário x dia(s), com clique no horário livre abrindo o cadastro de reserva e clique numa reserva abrindo os detalhes. A visão **Mês** entra como uma terceira opção no mesmo seletor, sem alterar Dia/Semana.
- **Mês** mostra o calendário tradicional (semanas completas, segunda a domingo, incluindo os dias do mês anterior/seguinte que completam a primeira/última semana). Cada dia mostra até 3 reservas (horário + responsável, coloridas por esporte/status — mesma paleta da grade de hora) e "+N mais" quando há mais.
...
- **Detalhe do responsável** (`/dashboard/arenas/{id}/mensalistas/{athleteId}`):
  ...
  - **Recorrências:** quadra, dia, horário, valor/mês; toggle **Rateio**, botão **Reajustar valor**, ação **Pausar plano** e ação **Encerrar** por recorrência. Quando há encerramento previsto, um destaque mostra o mês, a observação e o horário que ficará vago (para revenda).
...
- **Rateio da mensalidade (incremental — revisado em 13/09/2026):**
  ...
  - Cada pagamento abate do valor total devido. A tela sempre mostra o resumo **Pago / Falta**. Pagar acima do que falta continua permitido; o excedente pode virar **crédito** do atleta (mesma regra do pagamento avulso — ver abaixo), agora calculado sobre o que falta da mensalidade inteira, não da fatia de quem está pagando.
  - Desativar (toggle) um participante só é possível enquanto ele ainda não pagou nada.
...
- **Pausar plano (viagem/afastamento temporário):** botão **Pausar plano** grava um período (data início/fim) em que a recorrência libera o horário — ao contrário do encerramento, é **reversível**: a recorrência volta a gerar reservas e mensalidades normalmente assim que o período termina, sem nenhuma ação manual.
  ...
  - O gestor também escolhe o que fazer com **reservas já marcadas** dentro do período: **liberar** (cancela, deixando o horário livre para outras reservas) ou **manter** (preserva as sessões já combinadas, mesmo com a pausa registrada).
  - Nenhuma reserva nova nasce dentro do período pausado — a agenda do espaço mostra o horário livre automaticamente, sem tratamento especial na tela de calendário.
  - Só é possível **remover** ou **editar** uma pausa que **ainda não começou** (mesma regra da previsão de encerramento); uma pausa em andamento não pode ser encerrada antecipadamente nesta versão. Remover reverte o valor da mensalidade e reativa, quando possível, as reservas que a própria pausa havia liberado.
  - Uma recorrência pausada mostra o selo **Pausado** na lista e no detalhe. Histórico de pausas (período, modo de cobrança, ação nas reservas, observação) fica visível na recorrência, no mesmo espírito do histórico de reajustes.
...
- **O pró-rata considera o horário, não só a data (corrigido em 14/09/2026).** Cadastrar um mensalista de segunda 16h–17h numa segunda às 20h40 contava a sessão de hoje como cobrável, mesmo já tendo passado o horário de jogar — o "Cobrado agora" cobrava por uma sessão que o atleta não usa. Agora a sessão de hoje só entra na conta (mensalidade sugerida, pró-rata e "Cobrado agora") quando o horário do bloco ainda não começou; do contrário as reservas e a cobrança pulam para a próxima ocorrência, igual ao banco já fazia ao decidir quais reservas criar.
- **O valor mensal volta a acompanhar a grade ao adicionar blocos (corrigido em 14/09/2026).** Acrescentar um segundo horário à recorrência atualizava o subtotal, mas o campo "Valor mensal cobrado" ficava travado no valor do primeiro bloco — o gestor precisava editar à mão para não subcobrar. O preenchimento automático volta a somar cada novo bloco.
...
- **Checkbox "Detalhar por hora" — extrato de ocupação** (14/09/2026): resolve o fechamento de mês com professor/mensalista. Desmarcado, o relatório é o de sempre (mensalidade agrupada, uma linha por pagamento). Marcado, **cada reserva vira uma linha por hora ocupada**, com data, faixa de horário, espaço, esporte, valor e status:
  ...
  - **Valor da hora:** no mensal é a **mensalidade do mês rateada pelas horas reservadas** (desde 24/09/2026 — antes vinha da tabela de preço e a soma não batia com a cobrança); no avulso vale **o que foi realmente cobrado na reserva**, rateado entre as horas, para respeitar desconto/negociação. Só o plano antigo, sem mensalidade gerada, ainda usa a tabela de preço (com a tabela **Padrão** do espaço como rede quando a escolhida está sem grade).
...
- **Exportar PDF** (15/09/2026): ao lado de "Exportar Excel", um botão gera um PDF do recorte atual da tabela — paisagem, com cabeçalho contendo o **logo do Arena Digital** (`public/logo_arena_front_bgbranco.png`, centralizado e proporcional — banner horizontal, dimensionado pela largura), a assinatura "Sistema de Gestão para Arenas Esportivas" e, centralizados logo abaixo, **nome e endereço da arena** (telefone, e-mail e CNPJ/CPF ficaram de fora do cabeçalho — ajustado em 15/09/2026). Abaixo do cabeçalho, a lista de **todos os filtros aplicados** (Período sempre aparece; Tipo de Jogo, Atleta, Espaço, Esporte, Rateio e Detalhar por hora só quando de fato selecionados) e o **resumo do período** — os mesmos totais dos cards da tela (Confirmados, Pendentes, Cancelados, Total a cobrar) e, com um Atleta filtrado, quanto ele deve de Mensal/Avulso. Por fim, a tabela de lançamentos, com o status colorido como na tela (verde/amarelo/vermelho) e rodapé com data de geração e paginação.
  - **Sem "Horas ocupadas" (corrigido em 15/09/2026):** o resumo do PDF chegou a somar as horas ocupadas, mas o número só contava reservas avulsas e de mensalista **ainda sem mensalidade gerada no mês** — uma recorrência já consolidada (a maioria) não entrava, então o total ficava muito abaixo do real (2h exibidas para um mês com dezenas de horas de mensalistas). Removido do resumo do PDF em vez de mostrar um número que subestima o período. O mesmo problema existe no card "Total a cobrar" da tela (o texto "· Xh ocupadas"), ainda não corrigido lá.
- **A linha do mensalista é a mensalidade do mês (corrigido em 14/09/2026).** O mensal aparecia pela transação que o sistema lança na **criação** do plano, que carrega o valor mensal **cheio**: um plano de R$ 320/mês criado no meio de setembro — cuja primeira mensalidade é proporcional, R$ 240 — aparecia como **R$ 320, e já como "Pago"**, antes de qualquer recebimento, enquanto o card "{atleta} deve de Mensal" mostrava corretamente R$ 240 em aberto. Agora a linha vem da **cobrança** (a mesma fonte da tela de Mensalistas): valor da competência (proporcional na estreia, cheio a partir do mês seguinte) e status real — Pendente, Pago (com a data do recebimento) ou Cancelado. Pagamento parcial mostra o que falta; crédito aplicado quita como dinheiro.
  - Como consequência, o mês que já tem mensalidade gerada deixa de listar também **cada sessão** da recorrência — o mensalista deve uma mensalidade, não N reservas avulsas. O extrato de ocupação ("Detalhar por hora") segue mostrando sessão a sessão, como antes.
- **Correções de filtros e visão "quem deve quanto"** (24/09/2026, branch `fix/relatorio-pagamentos-reservas`):
  - **"Detalhar por hora" + Status mostrava só parte dos mensalistas (corrigido).** Com Perfil Mensalista e Status Pendente, a visão normal listava 6 mensalidades em aberto e o detalhamento só 1 atleta. O status da hora vinha da **reserva**, e o banco cria as reservas do 1º mês do plano já como confirmadas, antes de qualquer pagamento, então todo mundo que estreou no mês aparecia como "Pago". Agora a hora do mensalista herda o status da **mensalidade do mês**: quitada = Pago, em aberto/parcial = Pendente, plano cancelado ou sessão cancelada = Cancelado. As duas visões passam a concordar.
  - **Valor da hora do mensalista = mensalidade rateada** pelas horas reservadas no mês. A soma das horas bate ao centavo com o que o atleta deve, inclusive no mês de estreia proporcional. Sessão cancelada fica sem valor.
  - **Mensalidade zerada e sem pagamento** (ex.: pausa proporcional cobrindo o mês inteiro) aparece como **Cancelado**, não mais como "Pendente R$ 0,00".
  - **Visão Rateio:** plano cancelado deixa a recorrência e as partes como Cancelado, não mais como Pendente.
  - **Espaço/Esporte filtrado:** o mensalista passa a aparecer pela mensalidade (dos planos com bloco naquele espaço/esporte), não mais pelas reservas soltas sem preço.
  - **Coluna "Dia da semana"** aparece sempre que "Detalhar por hora" está marcado, seja qual for o Tipo de Jogo.
  - **Serviço com o mesmo nome nas duas visões:** a reserva de mensalista agora aparece como "Mensalista" (antes "Mensal").
  - **Resumo por atleta** (novo): tabela acima dos lançamentos com uma linha por atleta, com **Horas** (só no detalhamento), **Total do mês**, **Pago**, **Em aberto** e **Status** (Pendente = ainda tem saldo; Cancelado = tudo cancelado; senão Pago), mais uma linha de total. Respeita o filtro de Status, vem ordenada por quem deve mais e **toda coluna pode ser ordenada** com um clique no cabeçalho, igual à tabela de lançamentos (a ordem escolhida vale também para o Excel e o PDF). Cada linha tem os botões de WhatsApp e de extrato em PDF. Vai também para o **Excel** (aba "Resumo por atleta") e para o **PDF geral** (tabela antes dos lançamentos).
  - **PDF por atleta = extrato de uso:** o ícone de PDF (na linha do lançamento ou do resumo) gera o extrato que vai para o atleta conferir e pagar. O título é "Extrato de uso — {atleta} — {mês}", seguido de um bloco com horas no mês, total, pago, em aberto e situação, e **todas** as reservas hora a hora com data, dia da semana, horário, espaço, esporte, valor e status. Sai sempre detalhado por hora e com **todos os status**, ignorando o filtro de Status da tela para o atleta ver tudo o que jogou, inclusive o que já pagou.
